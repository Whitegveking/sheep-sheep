...
from PyQt5.QtWidgets import QApplication, QPushButton, QWidget, QFrame, QLabel, QMessageBox, QVBoxLayout
from PyQt5.QtCore import pyqtSignal, pyqtBoundSignal, Qt, QTimer  # 确保这里导入了 QTimer
from resources import read_stuffs, Stuff
...
class Game:
    ...
    def __init__(self, difficulty='medium'):
        # 根据难度设置方块数量
        if difficulty == 'easy':
            self.cube_total_count = 10 * 3  # 简单难度
        elif difficulty == 'hard':
            self.cube_total_count = 30 * 3  # 困难难度
        else:
            self.cube_total_count = 20 * 3  # 默认中等难度
...
class MainWindow(QWidget):
    def __init__(self, difficulty='medium'):
        ...
        self.time_left = 60  # 设置初始倒计时时间为60秒
        self.timer.timeout.connect(self.update_timer)

        # 初始化显示倒计时的标签
        self.timer_label = QLabel(self)
        self.timer_label.setText(f"剩余时间: {self.time_left}秒")
        self.timer_label.move(20, 20)  # 调整位置
    ...
    def update_timer(self):
        ...
        if self.time_left <= 0:
            self.timer.stop()
            QMessageBox.information(self, '时间到！', '游戏结束！', buttons=QMessageBox.Yes)
            self.restart_game()
...
class StartWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.setWindowTitle('开始游戏')  # 设置窗口标题
        self.setGeometry(100, 100, 400, 300)  # 设置窗口大小和位置

        self.layout = QVBoxLayout()

        self.title_label = QLabel('欢迎来到游戏！', self)  # 创建一个标签显示欢迎信息
        self.title_label.setAlignment(Qt.AlignCenter)  # 设置文本居中
        self.layout.addWidget(self.title_label)

        # 添加难度选择按钮
        self.easy_button = QPushButton('简单', self)
        self.easy_button.clicked.connect(lambda: self.start_game('easy'))
        self.layout.addWidget(self.easy_button)

        self.medium_button = QPushButton('中等', self)
        self.medium_button.clicked.connect(lambda: self.start_game('medium'))
        self.layout.addWidget(self.medium_button)

        self.hard_button = QPushButton('困难', self)
        self.hard_button.clicked.connect(lambda: self.start_game('hard'))
    ...
    def start_game(self, difficulty):
        ...
        切换到主游戏界面
        """
        self.main_window = MainWindow(difficulty)  # 传递难度参数
        self.main_window.show()  # 显示主游戏窗口
        self.close()  # 关闭开始窗口


if __name__ == '__main__':
    app = QApplication(sys.argv)
    start_window = StartWindow()  # 创建开始窗口
    start_window.show()  # 显示开始窗口
    sys.exit(app.exec_())
